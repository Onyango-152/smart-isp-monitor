...
class DashboardProvider extends ChangeNotifier {
  ...
  List<DeviceModel> get devices {
    final sorted = [..._devices]..sort((a, b) {
        int _priority(String s) {
          if (s == AppConstants.statusOffline)  return 0;
          if (s == AppConstants.statusDegraded) return 1;
          return 2;
        }
        return _priority(a.status).compareTo(_priority(b.status));
      });
  ...
  List<({DeviceModel device, AlertModel? topAlert})> get needsAttention {
    final nonOnline = _devices
        .where((d) => d.status != AppConstants.statusOnline)
        .toList();
    ...
    for (final alert in _activeAlerts) {
      ...
      if (existing == null || _sevRank(alert.severity) < _sevRank(existing.severity)) {
  ...
  static int _sevRank(String severity) {
    switch (severity) {
      case AppConstants.severityCritical: return 0;
      case AppConstants.severityHigh:    return 1;
      case AppConstants.severityMedium:  return 2;
      case AppConstants.severityLow:     return 3;
      default:                           return 4;
    }
  }
  ...
  static int _attentionScore(String status, AlertModel? topAlert) {
    ...
    if (topAlert != null && topAlert.severity == AppConstants.severityCritical) return 1;
    // Degraded + high alert
    if (topAlert != null && topAlert.severity == AppConstants.severityHigh) return 2;
    // Degraded + medium/low alert
    if (topAlert != null) return 3;
    // Degraded, no alerts
    return 4;
  }
  ...
  double get mttrMinutes {
    final resolved = _activeAlerts
        .where((a) => a.isResolved && a.resolvedAt != null);
    if (resolved.isEmpty) return avgMttrMinutes.toDouble();
    final totalMin = resolved.fold<double>(0, (sum, a) {
      final diff = DateTime.parse(a.resolvedAt!).difference(DateTime.parse(a.triggeredAt));
